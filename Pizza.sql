...
SELECT * FROM ORDERS;
 -- Highest Hourly Order, 12 PM   
with cte as (select 
hour (str_to_date(time, '%H:%i:%s')) AS TIMESS, order_id
from orders)
select timess, count(order_id) as Total_orders
from cte
group by 1
order by 2 desc;

-- top 5 maximum order date 
WITH CTE AS(
select date, count(order_id) as daily_sales,
 row_number () over(order by count(order_id) desc) as RN
 from orders
 group by date)
 select * 
 from cte
